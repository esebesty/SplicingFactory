...
test_that("Basic input error handling is working.", {
    ...
    for (method in c("naive", "laplace", "gini", "simpson", "invsimpson")) {
        ...
        expect_message(calculate_diversity(count_matrix, genes, method, norm, tpm = TRUE), "Note: tpm as a logical argument is only interpreted in case of
            tximport lists.")
        ...
        for (method in c("gini", "simpson", "invsimpson")) {
            ...
            expect_message(calculate_diversity(count_matrix, genes, method = "gini", norm = FALSE, tpm))
        }
    }
})
